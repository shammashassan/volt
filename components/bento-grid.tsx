...
export const BentoCard = ({ children, className, title, description, icon, image, href, priority }: BentoCardProps) => {
  const CardContent = (
    <div
      className={cn(
        "group relative flex h-full flex-col justify-between overflow-hidden rounded-3xl border bg-background p-6 shadow-sm transition-[box-shadow,background-color,border-color] duration-300 hover:shadow-md",
        href && "cursor-pointer",
        className
      )}
    >
      {/* Inner wrapper to handle translation on hover to prevent GSAP transform conflicts on the outer div */}
      <div className="flex flex-col justify-between h-full w-full transition-transform duration-300 group-hover:-translate-y-1.5">
        <div className="pointer-events-none z-10 flex flex-col gap-1 transition-transform duration-300 group-hover:-translate-y-0.5">
