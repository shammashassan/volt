...
export function StatsCard({ resourcesCount, categoriesCount, favoritesCount, inboxCount }: StatsCardProps) {
    ...
    return (
        <Card className="flex flex-col justify-between h-full border-border/50 bg-card/60 p-5 shadow-sm backdrop-blur-sm gap-5">
            {/* Stats Grid */}
            <div className="grid grid-cols-3 divide-x divide-border/40">
                {items.map((item, idx) => (
                    <div key={idx} className="flex flex-col gap-2.5 px-3 first:pl-0 last:pr-0">
                        {/* Top: Icon wrapper */}
                        <div className={`rounded-md p-1 w-fit ${item.bgClass}`}>
                            <item.icon className={`size-3.5 ${item.colorClass}`} />
                        </div>
                        {/* Bottom: Count and Label stacked vertically */}
                        <div className="flex flex-col">
                            <span className="font-mono text-2xl font-bold tracking-tight text-foreground tabular-nums">
                                {item.value}
                            </span>
                            <span className="font-mono text-[10px] lowercase tracking-wider text-muted-foreground/70 animate-[pulse_4s_ease-in-out_infinite]">
                                {item.label}
                            </span>
                        </div>
                    </div>
            ...
            <div className="flex flex-col gap-3">
                ...
                {/* Sync Status */}
                <div className="flex items-center justify-between text-[10px] font-mono lowercase tracking-wider text-muted-foreground/60">
