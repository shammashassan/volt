...
export function Preloader({ onComplete, onExitStart }: PreloaderProps) {
  ...
  useGSAP(() => {
    ...
    // ── ambient ring rotations (GPU-accelerated HTML container wrappers) ──────
    const outerSpin = gsap.to('.ring-outer-wrap', {
      rotation: 360,
      duration: 14,
      ease: 'none',
      repeat: -1,
    ...
    const innerSpin = gsap.to('.ring-inner-wrap', {
      rotation: -360,
      duration: 8,
      ease: 'none',
      repeat: -1,
    ...
    const logoBreath = gsap.to('.logo-mark', {
      opacity: 0.55,
      duration: 1.9,
      ease: 'sine.inOut',
      repeat: -1,
    ...
    tl.to(
      progressObj,
      {
        ...
        onUpdate() {
          ...
          // Status copy — update directly on boundary cross (no nested GC/tween overhead)
    ...
    tl.to(
      ['.ring-outer-wrap', '.ring-inner-wrap'],
      {
        rotation: (i: number) => (i === 0 ? '+=15' : '-=15'),
        duration: 0.3,
        ease: 'power4.out',
      },
    )
    ...
    // Radial glow pulse
    tl.to('.center-glow', { opacity: 1, scale: 1.25, duration: 0.45, ease: 'power2.out', transformOrigin: '50% 50%' })
    tl.to('.center-glow', { opacity: 0, duration: 0.6, ease: 'power2.in' }, '+=0.15')

    // Brief "complete" state — arc flashes white
    tl.to('.progress-arc', { strokeOpacity: 0, duration: 0.35, ease: 'power2.in' }, '-=0.3')

    // ── unified exit ──────────────────────────────────────────────────────────
    // 1. Content collapses inward (overlay layers) - pure opacity fade to prevent SVG re-rasterization jank
    tl.to(
      ['.overlay-content', '.hud-corner'],
      { opacity: 0, duration: 0.4, ease: 'power2.inOut' },
    )

    // Trigger page reveal animations slightly before panels slide apart
    tl.add(() => {
      onExitStart?.()
    }, '-=0.15')

    // 2. Panels slide apart — they carry the "door halves" of the circle
    tl.to('.panel-left', { xPercent: -100, duration: 1.25, ease: 'power4.inOut' }, '-=0.08')
    tl.to('.panel-right', { xPercent: 100, duration: 1.25, ease: 'power4.inOut' }, '<')

    // 3. Dissolve the container so the dashboard is fully revealed
    tl.to(root, { opacity: 0, duration: 0.4, ease: 'power2.in' }, '-=0.35')
  ...
  return (
    ...
    >
      ...
      <div className="panel-left  absolute inset-y-0 left-0  w-1/2 bg-[#060608] will-change-transform" />
      <div className="panel-right absolute inset-y-0 right-0 w-1/2 bg-[#060608] will-change-transform" />
      ...
      <div className="overlay-content absolute inset-0 flex flex-col items-center justify-center gap-7 pointer-events-none">
        ...
        <div className="center-assembly relative w-[196px] h-[196px] flex items-center justify-center" style={{ opacity: 0 }}>
          ...
          {/* Static elements (progress track, progress arc, center fill, ticks) */}
          ...
          {/* Outer dashed decoration — GPU-accelerated rotating wrapper */}
          <div className="ring-outer-wrap absolute inset-0 will-change-transform">
            <svg
              viewBox="0 0 100 100"
              className="w-full h-full"
              aria-hidden="true"
          ...
          {/* Inner dashed decoration — GPU-accelerated counter-rotating wrapper */}
          <div className="ring-inner-wrap absolute inset-0 will-change-transform">
            <svg
              viewBox="0 0 100 100"
              className="w-full h-full"
              aria-hidden="true"
